...
describe('OAuth Authorization', () => {
  ...
  describe('extractFieldFromWwwAuth', () => {
    function mockResponseWithWWWAuthenticate(headerValue: string): Response {
      return {
        headers: {
          get: jest.fn((name) =>
            name === 'WWW-Authenticate' ? headerValue : null,
          ),
        },
    ...
    it('returns the value of a quoted field', () => {
      const mockResponse = mockResponseWithWWWAuthenticate(
        `Bearer realm="example", field="value"`,
      );
    ...
    it('handles fields with special characters in quotes', () => {
      ...
      expect(extractFieldFromWwwAuth(mockResponse, 'error_description')).toBe(
        'The token has expired, please re-authenticate.',
      );
    });
  });
  ...
  describe('extractWWWAuthenticateParams', () => {
    ...
    it('returns resource metadata url when present and quoted', async () => {
      const resourceUrl =
    ...
    it('returns resource metadata url when present and unquoted', async () => {
      const resourceUrl =
        'https://resource.example.com/.well-known/oauth-protected-resource';
      const mockResponse = mockResponseWithWWWAuthenticate(
        `Bearer realm=mcp, resource_metadata=${resourceUrl}`,
    ...
    it('returns scope when present and quoted', async () => {
      const scope = 'read';
      const mockResponse = mockResponseWithWWWAuthenticate(
    ...
    it('returns scope when present and unquoted', async () => {
      const scope = 'read';
      const mockResponse = mockResponseWithWWWAuthenticate(
        `Bearer realm=mcp, scope=${scope}`,
      );
    ...
    it('returns both resourceMetadataUrl and scope when both are present', async () => {
      const resourceUrl =
        'https://resource.example.com/.well-known/oauth-protected-resource';
    ...
    it('handles multiple parameters in WWW-Authenticate header', async () => {
      const mockResponse = mockResponseWithWWWAuthenticate(
        `Bearer realm="api", error="invalid_token", error_description="The access token expired", scope="admin"`,
      );
      expect(extractWWWAuthenticateParams(mockResponse)).toEqual({
        scope: 'admin',
      });
    ...
    it('returns empty object if WWW-Authenticate header is missing', async () => {
      const mockResponse = {headers: new Headers()} as unknown as Response;
      expect(extractWWWAuthenticateParams(mockResponse)).toEqual({});
    });
    ...
    it('returns empty object if resource_metadata and scope not present', async () => {
      const mockResponse =
        mockResponseWithWWWAuthenticate(`Bearer realm="mcp"`);
      expect(extractWWWAuthenticateParams(mockResponse)).toEqual({});
    });
